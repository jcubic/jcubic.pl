...
trakcie nauki. Tutaj masz w jednym miejscu 20 punktów, które oprócz HTML, CSS oraz JavaScript
powinieneś znać, gdy zajmujesz się tworzeniem stron lub aplikacji internetowych.
...
zbudowany. Np. adres 127.0.0.1 jest to adres twojego komputera tzw. pętla zwrotna (ang. loopback).

[Więcej o adresach IP na Wikipedii](https://pl.wikipedia.org/wiki/Adres_IP)

## 2. Pakiety oraz TCP/IP.
Nie jest to bardzo potrzebne w tworzeniu stron, ale warto sobie zdawać z sprawę z tego jak
...
Jest to nazwa serwera, który identyfikuje go w internecie.
Domeny zarządzane są przez system DNS (ang. Domain Name System), służy on do tego,
aby zamieniać adresy IP serwera na przyjazne nazwy np. google.pl, nie musisz pamiętać
jaki ma adres IP tylko używasz nazwy, dzięki temu np. jedna domena może wskazywać na
kilka adresów IP można też je zmienić i użytkownicy tego nie zauważą.
Serwer lokalny np. jest nazywany localhost, ale ta nazwa nie jest zapisana
w systemie DNS tylko w systemie operacyjnym. Możesz sobie np. dodać do własnego
użytku różne nazwy dla localhost, np. dev.domena.pl. Lokalne nazwy będą nadpisywały
te z systemu DNS. Więcej o [DNS na Wikipedii](https://pl.wikipedia.org/wiki/Domain_Name_System).

## 4. Serwer

Istnieją dwa rodzaje serwerów. Fizyczna maszyna oraz program, który nasłuchuje na odpowiednie dane
przesyłane przez TCP/IP i zwraca odpowiednio przetworzone dane. Może być np. serwer www, który
odbiera informacje jakie strony otworzyć (zazwyczaj z przeglądarki) i zwraca zawartość
strony.
...
Aplikacje składające się z serwera i klientów nazywane są aplikacjami klient-serwer.
...
internetowych, będę pisał w następnym artykule.

Funkcje jakie ma [Atthost](https://ref.atthost.pl/?id=10912) to SSH, darmowy SSL, SSD czyli szybie
dyski, FTP, prosto konfigurowalne aplikacje w Node.js, Ruby on Rails, Python - Django czy Flask,
...
dostane jakieś grosze, dzięki którym będę miał kasę na utrzymanie tej strony.
...
www. [Atthost](https://ref.atthost.pl/?id=10912) udostępnia oba. SSH czasami nie jest dostępny, a
dzięki niemu można się "przenieść" do systemu operacyjnego na serwerze (czyli GNU/Linuxa i wykonywać
tam polecenia, co jest bardzo użyteczne. Na poprzednim hostingu tego nie miałem i napisałem specjalnie
aplikację, która udostępnia wiersz poleceń o nazwie [Leash](https://leash.jcubic.pl/),
jeśli masz już hosting które nie ma SSH możesz z niej skorzystać lub przejść do Atthost).
...
można wykonać zapytanie http łącząc się portem 80. Nie można jednak się pomylić i skasować tego
...
korzystasz ze starszego Windowsa (przed Windows 10) ponieważ tam masz dostęp do prawdziwego Linuxa.
...
Linux to super narzędzie programistyczne, jest o wiele lepszy niż MacOS oraz Windows. Chociaż MacOS
to też unix, a Windows 10 udostępnia WSL. Jeśli masz Windowsa 10 koniecznie zainstaluj WSL (czyli
Windows Subsystem for Linux). Ważne jest to także, gdy zainteresujesz się
[Dockerem](https://pl.wikipedia.org/wiki/Docker_(oprogramowanie)), w skrócie jest to środowisko do
umieszczania aplikacji w wirtualnym systemie Linux, na dowolnym systemie. Przyda się także do
obsługi systemu kontroli wersji [GIT](https://pl.wikipedia.org/wiki/Git_(oprogramowanie)). Oba
narzędzia raczej dla programistów, albo dla tych, którzy chcą się rozwijać.
...
określany jako liczba od 0 do 65535. Np. jak otwierasz stronę www w przeglądarce, to pobierasz ją z
portu 80, którym nasłuchuje serwer www (lub 443 dla https).  Serwery aplikacji mają zazwyczaj
...
porcie, dlatego trzeba go podawać gdy się go używa. Jak używać portów w adresie w następnym punkcie
o adresach URL. Więcej o portach przeczytasz na
...
[https://jcubic.pl/search.php?q=serwer](https://jcubic.pl/search.php?q=serwer) jest to coś co masz w
pasku adresu przeglądarki, taki adres dostaniesz, gdy wyszukasz słowo "serwer" na tej stronie.

Jeśli serwer www byłby np. na porcie 8080 pod adresem domeny musiałbym otworzyć stronę
http://jcubic.pl:8080, dla połączenia nieszyfrowanego. Można mieć więcej niż jeden serwer www
na jednej maszynie, każdy na innym porcie. Często się zdarza np. że port 80 jest zajęty i odpala
...
Proste zapytanie HTTP wygląda tak (dla przykładu wyszukiwania słowa "serwer" na mojej stronie)
...
chodzi. Może to być np. GET, POST, HEAD lub PUT. Potem adres strony i wersja protokołu. Dalej są
...
poszuka nagłówka location i otworzy tą stronę (czyli przekieruje na ten adres). Dalej jest jedna
pusta linia (czyli dwa znaki końca wiersza \n\n) i strona HTML, jest ona po to, aby
użytkownik coś zobaczył, gdy jego przeglądarka nie zrozumie, że jest to przekierowanie. Wtedy
...
sesji (stan nie jest zawarty w samym protokole tylko w aplikacji po stronie serwera która używa HTTP).
Można np. napisać aplikacje, gdzie można się zalogować i każda następna strona
będzie pamiętać, że jesteś zalogowany. Jest to zazwyczaj realizowane dzięki ciasteczkom.
Ciasteczko jest to po prostu pewien tekst wysyłany do przeglądarki, który jest za każdym
nowym zapytaniem zwracany do serwera. W ciasteczkach można zapisywać informacje,
w formacie klucz wartość. Np. można zapisać, że id_użytkownika to 111234567.
...
strony muszą wyświetlać informacje i używaniu ciasteczek. Mogą także prowadzić do problemów
...
zupełnie inna niż źródło strony.
...
własnościowego i płatnego programu Ilustrator od Adobe. Jest zupełnie darmowy i dostępny na
Windowsa, Linuxa i MacOS. Program do grafiki wektorowej jest moim zdaniem o wiele lepszy niż do
grafiki rastrowej taki jak Adobe Photoshop czy GIMP.
...
Polecam też książkę
["Design dla hakerów. Sekrety genialnych projektów"](https://helion.pl/ksiazki/design-dla-hakerow-sekrety-genialnych-projektow-david-kadavy,deshak.htm#format/e). Niestety w księgarni Helion tylko ebook,
prawdopodobnie nikt z drukowaną kopią się nie rozstaje, jest tak dobra.
...
prawdopodobne, że będziesz miał jakieś problemy. Wtedy możesz szukać pomocy, jeśli nie znajdziesz
odpowiedzi w Google. Będziesz musiał zadać pytanie. Dobrym miejscem są np. grupy na Facebooku lub
...
odpowiedniego nazywania tego problemu. Tutaj pomocna jest tylko praktyka.
...
rekrutacji. Ja zatrudniając kogoś na pewno bym poprosił o link do konta, jeśli kandydat takie ma
...
Znasz jeszcze coś, czego warto się nauczyć, gdy poznajesz sposoby tworzenia stron internetowych
